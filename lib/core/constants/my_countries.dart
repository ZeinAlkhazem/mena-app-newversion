/// [MayyaCountries] has a static method [countryList] that returns list of Map for most country around the world.

import 'package:intl_phone_number_input/src/models/country_list.dart';
import 'package:intl_phone_number_input/src/models/country_model.dart';
import 'package:mena/models/my_models/country_model.dart';

const String PropertyName = 'alpha_2_code';

/// [MayyaCountryProvider] provides helper classes that involves manipulations.
/// of Countries from [Countries.countryList]
class MayyaCountryProvider {
  ...
  static List<MayyaCountry> getCountriesData({required List<String>? countries}) {
    List jsonList = Countries.countryList;

    if (countries == null || countries.isEmpty) {
      return jsonList.map((country) => MayyaCountry.fromJson(country)).toList();
    }
    List filteredList = jsonList.where((country) {
      return countries.contains(country[PropertyName]);
    }).toList();

    return filteredList.map((country) => MayyaCountry.fromJson(country)).toList();
  }
}


class MayyaCountries {
